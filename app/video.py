...
class Embeddings_Store:
    ...
    def find_embedding(self, embedding: np.ndarray, metric: str='cosine', threshold: float=0.5, return_id: bool = False):
        if isinstance(embedding, np.ndarray):
            ...
            # print(distance_dict[best_id])
            if distance_dict[best_id] < threshold:
                return (best_id, distance_dict[best_id]) if return_id else (self.embedding_dict[best_id].get_name(), distance_dict[best_id])
            else:
                return None
        else:
...
while True:
    ...
    for face in faces:
        ...
        if detected_face is not None:
            box = face.bbox.astype(int)  # bbox: [x1, y1, x2, y2]
            text = unicodedata.normalize("NFD", detected_face[0])
            text_no_diacritics = ''.join(c for c in text if unicodedata.category(c) != 'Mn')
            cv2.putText(frame, text_no_diacritics, (box[0], box[1] - 10), cv2.FONT_HERSHEY_SIMPLEX, 0.6, (0, 255, 0), 2)
            cv2.putText(frame, str(detected_face[1]), (box[0], box[3] + 10), cv2.FONT_HERSHEY_SIMPLEX, 0.6, (0, 255, 0), 2)
            cv2.rectangle(frame, (box[0], box[1]), (box[2], box[3]), (0, 255, 0), 2)

    # 5. Hiển thị khung hình
